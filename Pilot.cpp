...
const hlt::Planet* Pilot::find_planet_todock(const hlt::Map& map) const {    
    std::vector<hlt::Planet> planets = DistanceSorter(_ship).planets_by_distance(map);
...
void Pilot::move_to_dock(const hlt::Map& map, hlt::Moves& moves) const {
    const hlt::Planet* planet = find_planet_todock(map);
    if (planet!=NULL){
